//! The automation schedule invoker: a periodic tick loop that fires due schedules
...
pub struct ResolvedFire {
    ...
    pub assignments: wardian_core::models::AutomationAssignments,
}
...
/// path (`commands::automation::resolve_blueprint_path`, backed by
/// `wardian_core::automation::resolve_blueprint_path`), so a scheduled run can
/// find a blueprint nested in a library subfolder, not just one that sits
/// flat in `library/automations` with a filename matching its id.
pub fn resolve_fire(req: &FireRequest) -> Result<ResolvedFire, String> {
    let path = wardian_core::automation::resolve_blueprint_path(&req.blueprint_id)
...
async fn fire_request(app: &AppHandle, req: &FireRequest) {
    let resolved = match resolve_fire(req) {
        ...
        Err(message) => {
            ...
            match record_schedule_launch_failure(req, &message) {
                Ok(run_root) => {
                    runs::emit_automation_inbox_update_with_name(app, &req.name, &run_root)
                }
                Err(error) => {
                    crate::utils::logging::log_debug(&format!(
                        "[automation] scheduler could not write failed run artifact: {error}"
                    ));
                }
            }
            return;
    ...
    crate::utils::logging::log_debug(&format!(
        "[automation] scheduler firing '{}' -> blueprint {} run {}",
    ...
    tokio::spawn(async move {
        ...
        match result {
            ...
            Err(error) => {
                crate::utils::logging::log_debug(&format!(
                    "[automation] scheduled run failed: {error}"
                ));
...
fn mark_error(schedule_id: &str, message: &str) {
    ...
    crate::utils::logging::log_debug(&format!("[automation] scheduler: {message}"));
}

fn record_schedule_launch_failure(req: &FireRequest, message: &str) -> Result<PathBuf, String> {
    let run_id = wardian_core::engine::driver::new_run_id();
    let run_root = wardian_core::paths::automation_run_dir(&req.blueprint_id, &run_id)
...
fn persist_runtime(
    processed: &[wardian_core::models::AutomationSchedule],
...
pub async fn start_scheduler(app: AppHandle) {
    let state = app.state::<crate::state::AppState>();
    {
        let mut handle = state.automation_scheduler_handle.lock().await;
    ...
    let mut slot = state.automation_scheduler_handle.lock().await;
    *slot = Some(handle);
}
...
"#;
    ...
    fn seed_blueprint(home: &std::path::Path, id: &str, text: &str) -> std::path::PathBuf {
        ...
        let path = automations_dir.join(format!("{id}.md"));
        std::fs::write(&path, text).unwrap();
        path
    }

    fn schedule(paused: bool, due_at: u64) -> wardian_core::models::AutomationSchedule {
        wardian_core::models::AutomationSchedule {
            id: "s1".into(),
    ...
    async fn due_schedule_fires_and_writes_a_run() {
        ...
        crate::automation::runs::drive_new_run(
            blueprint,
            run_id,
            run_root.clone(),
    ...
    fn resolve_fire_finds_a_blueprint_nested_in_a_library_subfolder() {
        ...
        // The library redesign made nested automation folders first-class:
    ...
    fn schedule_launch_failure_writes_a_failed_run_artifact() {
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set(home.path(), &mock_script_path());
        let req = fire_request_for("missing-automation", Some("mock"));
        ...
        let state = read_checkpoint(&run_root).unwrap().unwrap();
        assert_eq!(state.blueprint_id, "missing-automation");
        ...
        let update = runs::automation_inbox_update_with_name(&req.blueprint_id, &run_root).unwrap();
        assert_eq!(update.status, "failed");
        assert_eq!(update.automation_name, "missing-automation");
    }
