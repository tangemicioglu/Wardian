//! Generic conversation-boundary automation invoker.
...
pub struct SessionCloseContext {
    /// Unique lifecycle boundary identity. Retries of one launched automation
    ...
    for invoker in invokers {
        if !wardian_core::session_close::archive_requirement_satisfied(
            ...
            crate::utils::logging::log_debug(&format!(
                "[automation] session-close invoker {} skipped because no archive is available",
                invoker.id
            ));
            continue;
        }
...
async fn launch(
    ...
    let path = wardian_core::automation::resolve_blueprint_path(&invoker.blueprint_id)
        .ok_or_else(|| format!("could not resolve blueprint {}", invoker.blueprint_id))?;
    let blueprint =
        wardian_core::automation::parse_file(&path).map_err(|error| error.to_string())?;
    ...
    let run_root = wardian_core::paths::automation_run_dir(&blueprint.id, &run_id)
        .ok_or_else(|| "could not resolve automation run directory".to_string())?;
    ...
    tauri::async_runtime::spawn(async move {
        ...
        if let Err(error) = result {
            crate::utils::logging::log_debug(&format!(
                "[automation] session-close run failed: {error}"
            ));
        }
...
mod tests {
    ...
    fn partial_deterministic_run_is_repaired_before_it_is_claimed() {
        let temp = tempfile::tempdir().expect("temporary automation root");
