...
pub fn notify(app: Option<&tauri::AppHandle>, req: &NotifyRequest) -> Result<(), StepError> {
    ...
    if let Some(app) = app {
        ...
        app.notification()
            .builder()
            .title("Wardian automation")
            .body(&req.message)
            .show()
            .map_err(|error| StepError::new(format!("automation notification failed: {error}")))?;
    }
    Ok(())
}
...
mod tests {
    ...
    async fn shell_nonzero_exit_fails_the_step() {
        ...
        let err = run_shell(std::path::Path::new("."), &req)
            .await
            .expect_err("non-zero shell exit should fail the automation step");
...
"#,
    ...
    async fn script_nonzero_exit_fails_the_step() {
        ...
        let err = run_script(temp.path(), &req)
            .await
            .expect_err("non-zero script exit should fail the automation step");
