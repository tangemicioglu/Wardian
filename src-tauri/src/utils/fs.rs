...
/// Renders only Wardian's managed common, class, and agent sources, in that
/// order. Workspace discovery and arbitrary include paths remain the provider's
/// responsibility; generated habitat files are never read back as authority.
pub(crate) fn managed_instruction_content(
    wardian_home: &std::path::Path,
    class_name: &str,
    session_id: Option<&str>,
) -> String {
    ...
    let agent_agents = session_id
        .filter(|sid| {
            let mut components = std::path::Path::new(sid).components();
            matches!(
