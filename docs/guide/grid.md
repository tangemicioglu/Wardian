...
Chat mode shows normalized user, assistant, status, tool, approval, and terminal-output events. Use `Enter` to send and `Shift+Enter` for a newline. The composer accepts file-picker selections, native file drops, and filesystem-backed file pastes. Changing its model persists the selection and applies both model and effort to a live provider; for Codex, Wardian completes the interactive `/model` pickers step by step. An off agent uses the saved choice after its next start or restart. Model and effort controls remain unavailable until the operation finishes, so each change is applied in order. Switch back to Terminal when you need provider-specific keys or a raw approval screen.
