...
`send_message` routes provider-aware delivery to one or more targets. Delivery responses contain `runtime_state`, `delivery_state`, `input_mode`, optional `message_id`, `delivery_phase`, `observed_state`, `reason`, `profile`, and provider-specific error details. Each send has a durable interaction and delivery attempt. When the target runtime is not ready for live input, the command queues the message in the mailbox or fails according to its queue policy instead of injecting text early. Pending mailbox records survive restart and drain at a later idle or ready observation; they do not run on a timer or expire by age. Native message and command delivery is `provider_accepted` only after the PTY writer acknowledges the bytes and the provider emits a post-submit turn-start event. If terminal state is ambiguous after input is accepted, the command records a failed delivery rather than replaying the message.
