...
- Repeated telemetry `Idle` samples are not Ready observations. They cannot
  advance that sequence or drain a mailbox. A telemetry-observed transition
  into `Idle` has a distinct status-observation sequence and can release one
  record, as can a provider-status observation or provider readiness signal.
...
- During startup recovery, legacy mailbox rows are durably armed with the
  ...
  interaction, and records its terminal delivery attempt. The queue-if-busy
  delivery specification records the full causal-ordering contract.
