...
Wardian ships five editable automation templates under
`<WARDIAN_HOME>/library/automations/samples/`. They are copied only when missing,
so an edit to a sample remains yours. Samples never run or create schedules on
their own.

| Template | Use it for | Automation shape |
| --- | --- | --- |
...
The samples are genericized from local automation structures such as review,
