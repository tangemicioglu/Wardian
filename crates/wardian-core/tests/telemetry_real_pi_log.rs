...
fn declared_usage(path: &std::path::Path) -> Vec<serde_json::Value> {
    let content = std::fs::read_to_string(path).expect("fixture is readable");
    content
        .lines()
...
fn the_fixture_produces_usable_facts_and_survives_forced_reparse() {
    let snapshot = provider_log::ProviderLog::capture("pi", &fixture_log()).unwrap();
    assert_eq!(snapshot.verify().unwrap(), (20_640, 7_680, 0, 183, 98));
}
