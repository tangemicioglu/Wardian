use crate::automation::registry::node_types;
...
mod tests {
    ...
    fn schema_lists_every_node_type_with_fields_and_ports() {
        ...
        assert_eq!(types.len(), crate::automation::registry::node_types().len());
        ...
        assert!(task["fields"]
            ...
            .iter()
            .any(|f| f["id"] == "prompt"));
        let sub_automation = types.iter().find(|t| t["id"] == "sub_automation").unwrap();
        assert_eq!(sub_automation["supported"], false);
    }
