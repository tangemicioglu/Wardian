...
use crate::models::{
    AgentConversationMode, AutomationAssignments, AutomationRoleAssignment, BusyPolicy,
    InvocationKind,
};
...
pub fn validate_assignments(assignments: &AutomationAssignments) -> Result<(), String> {
    for (role, assignment) in assignments {
        if role.trim().is_empty() {
            return Err("assignment role names must not be empty".to_string());
        }
        match assignment {
            AutomationRoleAssignment::Agent { agent_id, .. } => {
...
pub fn legacy_bindings(assignments: &AutomationAssignments) -> HashMap<String, String> {
    assignments
        .iter()
        .map(|(role, assignment)| {
            let target = match assignment {
                AutomationRoleAssignment::Agent { agent_id, .. } => agent_id,
                AutomationRoleAssignment::TemporaryProvider { provider, .. } => provider,
            };
...
mod tests {
    ...
    fn provider_legacy_binding_becomes_temporary_provider() {
        ...
        assert_eq!(
            ...
            Some(&AutomationRoleAssignment::TemporaryProvider {
    ...
    fn agent_legacy_binding_becomes_current_conversation_with_invocation_default() {
        ...
        assert_eq!(
            ...
            Some(&AutomationRoleAssignment::Agent {
