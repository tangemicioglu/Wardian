...
pub fn diff(before: &Blueprint, after: &Blueprint) -> BlueprintDiff {
    let before_map: BTreeMap<&str, &crate::automation::blueprint::Node> =
        before.nodes.iter().map(|n| (n.id.as_str(), n)).collect();
    let after_map: BTreeMap<&str, &crate::automation::blueprint::Node> =
...
mod tests {
    use super::*;
    use crate::automation::blueprint::{Blueprint, Node};
