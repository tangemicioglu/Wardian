use crate::automation::blueprint::Blueprint;
...
pub fn parse_str(text: &str) -> Result<Blueprint> {
    ...
    let after_open = trimmed
        ...
        .ok_or(AutomationError::MissingFrontMatter)?;

    let (yaml, body) = split_front_matter(after_open).ok_or(AutomationError::MissingFrontMatter)?;

    let mut blueprint: Blueprint =
        serde_norway::from_str(yaml).map_err(|e| AutomationError::Yaml(e.to_string()))?;
...
pub fn to_string(blueprint: &Blueprint) -> Result<String> {
    let yaml = serde_norway::to_string(blueprint)
        .map_err(|e| AutomationError::Serialize(e.to_string()))?;
...
mod tests {
    ...
    fn missing_front_matter_is_an_error() {
        ...
        assert!(matches!(
            err,
            crate::automation::AutomationError::MissingFrontMatter
        ));
    }
