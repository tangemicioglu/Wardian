...
/// caller's declared workspace. An automation can dispatch a registered agent or
...
///
/// The module and on-disk lock path retain their automation names so a newly
/// started Wardian process still coordinates with already-running versions
/// that only acquire this guard for automation drives.
...
fn open_lock_file() -> Result<File, String> {
    ...
    let path = home.join("runtime").join("automation-execution.lock");
...
mod tests {
    ...
    fn worktree_mutation_lock_blocks_another_wardian_process() {
        const CHILD_ENV: &str = "WARDIAN_TEST_AUTOMATION_LOCK_CHILD";
        const READY_ENV: &str = "WARDIAN_TEST_AUTOMATION_LOCK_READY";
        const TEST_NAME: &str =
            "automation_execution_lock::tests::worktree_mutation_lock_blocks_another_wardian_process";
        ...
        let mut child =
            std::process::Command::new(std::env::current_exe().expect("current test executable"))
                ...
                .spawn()
                .expect("spawn child automation lock holder");
        ...
        assert!(
            child
                .wait()
                .expect("wait for child automation lock holder")
                .success(),
            "child automation lock holder should exit cleanly"
        );
    }
}
