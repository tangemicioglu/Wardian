...
/// Held while a parked automation run transitions through an approval decision.
///
...
fn reserve_run(run_root: &Path) -> Result<(), ApprovalDecisionLockError> {
    let mut locked_runs = LOCKED_RUNS.lock().map_err(|_| {
        ApprovalDecisionLockError::Io(io::Error::other(
            "automation approval lock state is poisoned",
        ))
    })?;
