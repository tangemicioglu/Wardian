#[test]
fn automation_replay_uses_the_run_snapshot_when_the_library_copy_is_missing() {
    ...
    let run_root = automation_run_root("snapshot-only", "run-1").unwrap();
    ...
    let rendered = automation_replay::render("snapshot-only", "run-1").unwrap();
...
#[test]
fn automation_replay_supports_a_scheduler_launch_failure_without_a_blueprint() {
    let temp = tempfile::tempdir().unwrap();
    let _env = TestWardianHome::new(temp.path());
    let run_root = automation_run_root("missing-automation", "run-1").unwrap();
    let message = "could not resolve blueprint path for missing-automation";
    let mut state = wardian_core::engine::RunState::new("run-1", "missing-automation");
    ...
    let rendered = automation_replay::render("missing-automation", "run-1").unwrap();
    let json: serde_json::Value = serde_json::from_str(&rendered).unwrap();
