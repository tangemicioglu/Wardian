...
- **Local review, not GitHub reviewer requests**: zero-blocker review means review by local agents, obtained through the `autoreview` workflow when available and otherwise routed to Wardian-Reviewer. Never request reviewers on GitHub. GitHub's review status is not the local review verdict.
