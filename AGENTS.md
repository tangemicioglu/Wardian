...
- **Review Has A Termination Criterion**: Every review finding is blocking or non-blocking, and deciding which is yours to make. Only a blocking finding justifies another round; a non-blocking one goes in a linked issue, not another commit. Review ends at zero blocking findings, not zero possible improvements — "are there more gaps" always answers yes, so it cannot be the stopping condition. Repeatedly auditing your own diff is not review, it is a loop; route to Wardian-Reviewer for the verdict instead of re-deriving one yourself. If you notice yourself starting another self-audit pass on a branch whose checks already pass, stop and report instead.
...
- **CI Readiness**: Before opening a PR, run the full verification suite (`npm run typecheck/lint/test`, the `check:` gates listed above, and `cargo clippy/fmt/test`) to ensure green status on GitHub Actions. A PR is "ready" only when four separately checkable facts hold, not when a status update says so: every required check is green, merge conflicts are resolved, a review verdict is on record with zero blocking findings, and the linked issue's behavior is reachable in normal operation without a human running an extra command. Verify each one directly before reporting a PR as ready.
