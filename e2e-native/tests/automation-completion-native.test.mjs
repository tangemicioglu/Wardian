...
function seedAutomation(harness, { automationId }) {
...
nodes:
  ...
  - id: agent-node-1
    ...
    fields:
      agent: role:worker
      prompt: Complete this automation node.
edges:
  - from: trigger
    to: agent-node-1
---
...
async function invokeAutomationRun(driver, { automationPath, sessionId, workspace }) {
...
async function readLatestAutomationState(harness, automationId, timeoutMs = 10000) {
  const logDir = path.join(harness.isolatedHome, "logs", "automations", automationId);
    ...
    if (fs.existsSync(logDir)) {
      ...
      if (statePath && eventsPath && fs.existsSync(statePath) && fs.existsSync(eventsPath)) {
        try {
          ...
          // The automation may still be writing the trace files.
        }
      }
    }
  ...
  assert.fail(`Timed out waiting for completed automation state in ${logDir}`);
}
...
test("automation detects live agent turn completion instead of timing out", { timeout: 180000 }, async (t) => {
  ...
  const providerSessionId = `automation-completion-agent-${runId}`;
  const sessionName = `Automation-Completion-Agent-${runId}`;
  ...
  await invokeAutomationRun(session.driver, {
    automationPath,
    sessionId,
    workspace: path.join(harness.repoRoot, "e2e-native"),
  });

  const trace = await readLatestAutomationState(harness, automationId);
