...
test("native blueprint commands parse, validate, and write round-trip", { timeout: 180000 }, async (t) => {
  ...
  const inputPath = path.join(automationsDir, "demo.md");
  writeFileSync(
    inputPath,
    `---
schema: 2
...
`,
  ...
  const parsed = await invokeTauri(session.driver, "automation_parse", { path: inputPath });
  ...
  const validation = await invokeTauri(session.driver, "automation_validate", { blueprint: invalid });
  ...
  const outputPath = path.join(automationsDir, "demo-written.md");
  const writeResult = await invokeTauri(session.driver, "automation_write", {
    path: outputPath,
  ...
  const reparsed = await invokeTauri(session.driver, "automation_parse", { path: outputPath });
