...
export class ErrorBoundary extends Component<{ children: ReactNode }, { hasError: boolean; error: unknown }> {
  ...
  componentDidCatch(error: unknown, errorInfo: ErrorInfo) { console.error("ErrorBoundary caught an error", error, errorInfo); }
  render() {
    if (this.state.hasError) {
      return (
        // The fatal-error screen must render when the theme stylesheet is the
        // thing that failed, so it cannot depend on a theme token resolving.
