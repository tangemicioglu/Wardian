...
import { DiagnosticsPanel } from '../features/automations/builder/DiagnosticsPanel';
import { NodeConfigForm } from '../features/automations/builder/NodeConfigForm';
import { NodeLibrary } from '../features/automations/builder/NodeLibrary';
...
import type { RunSummary } from '../features/automations/run/runTypes';
...
export function AutomationsView({ theme }: AutomationsViewProps) {
  ...
  const openScheduleEditor = async (schedule: AutomationSchedule) => {
    ...
    <div data-testid="automations-view" data-tour-target="automation-view" className="relative flex h-full min-h-0 flex-col overflow-hidden rounded-lg border border-wardian-border bg-[var(--color-wardian-bg)] text-primary">
        ...
        <div className={`automations-toolbar__actions flex shrink-0 items-center gap-2 ${mode === 'monitor' ? 'flex-nowrap' : ''}`}>
          ...
          {mode !== 'edit' ? (
            <button
              ...
              aria-label="Refresh automation view"
              title="Refresh"
            >
          ...
          <button
            type="button"
            data-tour-target="automation-run-button"
      ...
      {mode === 'edit' && (
        <div className="shrink-0 border-b border-wardian-border bg-[var(--color-wardian-card)] px-3 py-2">
          <OnboardingHint
            id="automation-authoring:v1"
            title="Build, validate, then run"
            actions={<DocsLink path="/guide/automations">Automation guide</DocsLink>}
          >
      ...
      <div className="automations-body grid min-h-0 flex-1" data-drawer-open={drawerOpen ? 'true' : 'false'}>
        <main className="automations-main h-full min-h-0 overflow-hidden p-3">
          {mode === 'edit' ? (
            <AutomationEditMode theme={theme} addNodeRequest={addNodeRequest} />
          ) : mode === 'observe' ? (
        ...
        {drawerOpen && (
          <aside className="automations-run-drawer min-h-0 overflow-y-auto border-l border-wardian-border bg-[var(--color-wardian-card)] p-3">
      ...
      {launchOpen && (blueprintPath || editSchedule) && (
        <div className="automations-launch-overlay wardian-dialog-overlay absolute inset-0 z-20 flex items-start justify-center overflow-hidden p-8">
          <RunLaunchDialog
...
interface AutomationEditModeProps extends AutomationsViewProps {
  addNodeRequest: number;
}

function AutomationEditMode({ theme, addNodeRequest }: AutomationEditModeProps) {
  const blueprint = useBuilderStore((state) => state.blueprint);
  const diagnostics = useBuilderStore((state) => state.diagnostics);
  const setBlueprint = useBuilderStore((state) => state.setBlueprint);
  ...
  return (
    <div data-testid="automations-edit-mode" className="relative flex h-full min-h-0 flex-col overflow-hidden rounded-lg border border-wardian-border bg-[var(--color-wardian-bg)]">
      <div className="automations-edit-body grid h-full min-h-0 flex-1" data-inspector-open={selectedNode ? 'true' : 'false'}>
        <section className="automations-canvas relative min-h-0">
          <BuilderCanvas
          ...
          />
          <div
            data-testid="automation-canvas-meta"
        ...
        {selectedNode ? (
          <aside data-testid="automation-inspector" className="automation-node-inspector flex min-h-0 flex-col overflow-hidden border-l border-wardian-border bg-[var(--color-wardian-card)]">
      ...
      {libraryOpen ? (
        <div className="automation-node-library-overlay absolute inset-0 z-30 flex items-start justify-center bg-[color-mix(in_srgb,var(--color-wardian-bg),transparent_20%)] p-8">
...
async function resolveScheduleBlueprint(schedule: AutomationSchedule, currentBlueprint: Blueprint | null): Promise<{
  ...
  try {
    ...
    while (!ref) {
      const result = offset > 0
        ? await invoke<BlueprintListResult>('automation_list_blueprints', { offset })
        : await invoke<BlueprintListResult>('automation_list_blueprints');
    ...
    const parsed = await invoke<{ blueprint: Blueprint; diagnostics: unknown[] }>('automation_parse', { path: ref.path });
