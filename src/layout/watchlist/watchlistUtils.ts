...
function timestampSortValue(timestamp: string | null | undefined): number {
  if (!timestamp) return Number.NEGATIVE_INFINITY;
  const parsed = Date.parse(timestamp);
  return Number.isFinite(parsed) ? parsed : Number.NEGATIVE_INFINITY;
}

export function latestValidQueryTimestamp(
  ...timestamps: Array<string | null | undefined>
): string | undefined {
  let latest: string | undefined;
  for (const timestamp of timestamps) {
    if (!timestamp) continue;
    if (!latest || timestampSortValue(timestamp) > timestampSortValue(latest)) {
      latest = timestamp;
    }
  }
  return latest;
  ...
  return [...agents].sort((a, b) => {
    ...
    switch (column_id) {
      ...
      case 'provider_model':
        ...
        break;
      case 'last_queried':
        av = timestampSortValue(latestValidQueryTimestamp(
          telemetry[a.session_id]?.last_query_timestamp,
          interactions[a.session_id],
        ));
        bv = timestampSortValue(latestValidQueryTimestamp(
          telemetry[b.session_id]?.last_query_timestamp,
          interactions[b.session_id],
        ));
        break;
      case 'status_label':
