...
import type { GardenAutomationUnit } from "./garden.types";
import { isActiveAutomationStatus, automationStatusColor } from "./gardenStatus";
...
interface AutomationUnitProps {
  unit: GardenAutomationUnit;
  selected: boolean;
  theme: GardenTheme;
...
export const AutomationUnit: React.FC<AutomationUnitProps> = ({
  unit,
  selected,
  theme,
  onSelect,
  ...
  const fill = resolveCssVar(automationStatusColor(unit.runStatus));
  const active = isActiveAutomationStatus(unit.runStatus);
