/**
 * Where an automation is actually deployed.
 *
 * A blueprint describes what an automation *needs* — `role:evolver` says it wants
 * whichever agent fills the evolver role. It does not say which agent that is,
 * and for most automations the blueprint never will: the binding is made when the
 * automation is deployed, and it lives in the schedule record rather than in the
 * document.
 *
 * That is the missing half of an automation's identity on the map. Reading only the
 * blueprint, the Evolver's three automations share nothing with the Evolver but a
 ...
 * most-referenced district wins, which is the honest answer for an automation that
 * genuinely spans two places.
 */
export function deploymentsByBlueprint(
  schedules: readonly AutomationScheduleRecord[],
