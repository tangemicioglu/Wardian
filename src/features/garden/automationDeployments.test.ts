...
import { deploymentsByBlueprint } from "./automationDeployments";

const EVOLVER = "6bc97063-1de9-4245-a90a-d7f4064fa5e0";

describe("deploymentsByBlueprint", () => {
  it("reads the agent a schedule deploys an automation onto", () => {
  ...
  it("ignores a throwaway agent spun up for one run", () => {
    ...
    // so it cannot place an automation.
    const map = deploymentsByBlueprint([
      {
  ...
  it("trusts a legacy binding only when it names an agent that exists", () => {
    ...
    // roster, and treating `"codex"` as an agent id would tie the automation to
    // nothing.
    const schedules = [
