...
describe('RunLaunchDialog', () => {
  ...
  it('prefills provider from settings and launches vian automation_run', async () => {
  ...
  it('requires a workspace before saving a schedule', async () => {
    ...
    expect(await screen.findByText('Workspace is required for scheduled automations.')).toBeInTheDocument();
    expect(invokeMock).not.toHaveBeenCalledWith('schedule_create', expect.anything());
  });

  it('keeps long automation forms inside a scrollable viewport-bounded dialog', async () => {
    render(
      <RunLaunchDialog
        path="/x/wf.md"
        blueprintId="wf"
  ...
  it('preserves provider and input when editing an existing schedule', async () => {
    ...
    const editSchedule: AutomationSchedule = {
      id: 's1',
      blueprint_id: 'wf',
      name: 'Nightly',
      provider: 'claude',
  ...
  it('lets an automation role bind to an active agent when launched', async () => {
  ...
  it('lets each automation role choose its own fresh provider', async () => {
    const blueprint: Blueprint = {
      schema: 2,
      id: 'wf',
      name: 'Automation',
  ...
  it('hides provider and workspace controls for automations with no provider-backed or workspace-backed nodes', async () => {
    const blueprint: Blueprint = {
      schema: 2,
      id: 'loop-test',
      name: 'Loop Test',
