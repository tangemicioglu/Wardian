...
import type {
  AutomationAgentConversation,
  AutomationAssignments,
  AutomationBusyPolicy,
  ScheduleDefinition,
  AutomationSchedule,
...
interface RunLaunchDialogProps {
  ...
  onCancel: () => void;
  onScheduled?: () => void;
  editSchedule?: AutomationSchedule;
}
...
export function RunLaunchDialog({
  ...
  const invocation = useMemo(() => automationInvocationFromBlueprint(blueprint), [blueprint]);
  ...
  const [roleConversations, setRoleConversations] = useState<Record<string, AutomationAgentConversation>>(
    () => initialRoleConversations(editSchedule?.assignments),
  );
  const [roleBusyPolicies, setRoleBusyPolicies] = useState<Record<string, AutomationBusyPolicy>>(
  ...
  const run = async () => {
    if (!launchProviderAvailable) {
      setProviderNote('Choose an installed provider before running this automation.');
      return;
    }
    ...
    try {
      ...
      const res = await invoke<{ ok: boolean; run_id?: string; diagnostics?: unknown[] }>('automation_run', {
        path,
  ...
  const saveSchedule = async () => {
    ...
    if (!launchProviderAvailable) {
      setProviderNote('Choose an installed provider before scheduling this automation.');
      return;
    }
    if (!workspace.trim()) {
      setError('Workspace is required for scheduled automations.');
      return;
    }
  ...
  return (
    ...
    >
      <div className="shrink-0">
        <h3 id="run-launch-dialog-title" className="mb-2 text-sm font-semibold">Run automation</h3>
        <div className="wardian-segmented-control mb-3" data-tour-target="automation-schedule-mode" role="radiogroup" aria-label="Launch mode">
      ...
      <div className="min-h-0 overflow-y-auto pr-1" data-testid="run-launch-dialog-body">
        {roles.length > 0 && (
          <div className="mb-3 grid gap-2 border-t border-wardian-border pt-3">
            <div>
              ...
              <p className="mt-0.5 text-[10px] text-muted">Choose an existing agent or an automation-owned temporary agent for each role.</p>
            </div>
            {roles.map((role) => {
              ...
              return (
                <div key={role.name} data-tour-target={role.name.trim().toLocaleLowerCase() === 'evolver' ? 'automation-evolver-assignment' : undefined} className="relative rounded border border-wardian-border bg-[var(--color-wardian-bg)] p-2">
                    ...
                    <div className="mt-2 grid grid-cols-2 gap-2">
                      <div>
                        ...
                        <select
                          ...
                          onChange={(event) => {
                            const value = event.currentTarget.value as AutomationAgentConversation;
                      ...
                      {(roleConversations[role.name] ?? 'current') === 'current' && (
                        <div>
                          ...
                          <select
                            ...
                            onChange={(event) => {
                              const value = event.currentTarget.value as AutomationBusyPolicy;
        ...
        {launchMode === 'schedule' && (
          <div className="mb-3 border-t border-wardian-border pt-3" data-tour-target="automation-schedule-editor">
      ...
      <div className="mt-3 flex shrink-0 justify-end gap-2 border-t border-wardian-border pt-3" data-testid="run-launch-dialog-actions">
        ...
        {launchMode === 'run' ? (
          ...
          <button
            type="button"
            data-tour-target="automation-save-schedule"
            className="wardian-button wardian-button--primary"
            disabled={busy || !launchProviderAvailable}
...
function collectAssignments(
  ...
  roleConversations: Record<string, AutomationAgentConversation>,
  roleBusyPolicies: Record<string, AutomationBusyPolicy>,
  roleModelSelections: Record<string, ModelSelection>,
  launchMode: 'run' | 'schedule',
  workspace: string,
): AutomationAssignments {
  const entries: Array<[string, AutomationAssignments[string]]> = roles.map((role) => {
...
function automationInvocationFromBlueprint(blueprint?: Blueprint | null) {
  if (!blueprint) {
...
function initialRoleTargets(bindings?: Record<string, string>, assignments?: AutomationAssignments) {
...
function initialRoleConversations(assignments?: AutomationAssignments) {
  return Object.fromEntries(
...
function initialRoleBusyPolicies(assignments?: AutomationAssignments) {
  return Object.fromEntries(
    Object.entries(assignments ?? {})
      .filter(([, assignment]) => assignment.target_type === 'agent')
      .map(([role, assignment]) => [
...
function initialRoleModelSelections(assignments?: AutomationAssignments) {
  return Object.fromEntries(
    Object.entries(assignments ?? {})
