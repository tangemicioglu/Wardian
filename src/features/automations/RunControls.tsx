...
export function RunControls({
  ...
  const call = async (cmd: string, args: Record<string, unknown>) => {
    const action = cmd === 'automation_approve'
      ? args.granted === true ? 'approve' : 'reject'
      : cmd === 'automation_resume' ? 'resume' : 'cancel';
    setError(null);
    ...
    try {
      ...
      setError(`Could not ${action} this automation run: ${detail}`);
    } finally {
      ...
      <div className="flex gap-2">
        {status === 'awaiting_approval' && awaitingNode && (
          ...
          <button
            ...
            disabled={pendingAction !== null}
            onClick={() => call('automation_cancel', { blueprintId, runId })}
          >
        ...
        <button
          ...
          disabled={pendingAction !== null}
          onClick={() => call('automation_resume', { blueprintId, runId, blueprintPath })}
        >
