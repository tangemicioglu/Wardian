...
import type { AutomationAssignmentItem } from './assignmentPresentation';

interface AutomationAssignmentSummaryProps {
  automationName: string;
  items: AutomationAssignmentItem[];
  maxVisible?: number;
  compact?: boolean;
  expanded?: boolean;
...
export function AutomationAssignmentSummary({
  automationName,
  items,
  maxVisible = 2,
  compact = false,
  ...
  onExpandedChange,
}: AutomationAssignmentSummaryProps) {
  ...
  return (
    <div className="min-w-0">
      <div className="flex min-w-0 flex-wrap gap-1">
        ...
        {hiddenCount > 0 ? (
          <button
            ...
            aria-label={expanded
              ? `Hide additional agents for ${automationName}`
              : `Show ${hiddenCount} more agents for ${automationName}`}
      ...
      {expanded && hiddenCount > 0 ? (
        <div
          data-testid="expanded-automation-assignments"
