...
import type { ActivityTone, AutomationActivity } from './monitorModel';
import { scheduleSummaryLabel } from './scheduleStatus';
import { formatRunDuration, formatAutomationTime, runTimestampValue } from './automationTime';

export interface AutomationActivityCardProps {
...
function timeValue(
  ...
  const label = formatAutomationTime(value, { now, emptyLabel });
...
function Detail({ label, children }: { label: string; children: ReactNode }) {
  return (
    <div className="min-w-0">
      ...
      <dd className="automation-activity-card__detail-value mt-0.5 truncate text-[10px] text-[var(--color-wardian-text)]">{children}</dd>
    </div>
  );
}
...
function ModeDetails({ activity, schedule, now, compact = false }: {
  ...
  const detailsClass = `automation-activity-card__details grid gap-2${compact ? ' automation-activity-card__details--compact' : ''}`;
...
export function AutomationActivityCard({
  ...
  const testId = activity.section === 'history' && run
    ? `automation-history-run-${run.run_id}`
    : `automation-activity-row-${activity.blueprintId}`;
  ...
  return (
    ...
    >
      <header className="automation-activity-card__header flex items-start justify-between gap-3">
        <div className={`automation-activity-card__identity ${compactHistory ? 'flex min-w-0 flex-1 items-center gap-2' : 'min-w-0'}`}>
        ...
        <div className="automation-activity-card__actions flex shrink-0 items-center gap-1.5">
          {run ? (
            <button
              type="button"
      ...
      {assignments.length > 0 ? (
        ...
        >
          <AutomationAssignmentSummary
            automationName={activity.name}
            items={assignments}
            compact={virtualized}
      ...
      <div className={`automation-activity-card__details-shell ${virtualized ? 'mt-2 pt-1' : 'mt-3 pt-2'} border-t border-wardian-border`}>
