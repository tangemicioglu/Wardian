...
export interface AutomationTimeLabel {
  primary: string;
  exact: string | null;
  valid: boolean;
}

export interface AutomationTimeOptions {
  now?: Date;
  locale?: string;
  emptyLabel?: string;
}

type AutomationTimeValue = string | number | Date | null | undefined;

function localDaySerial(value: Date): number {
  return Date.UTC(value.getFullYear(), value.getMonth(), value.getDate()) / 86_400_000;
}

function originalLabel(value: Exclude<AutomationTimeValue, null | undefined>): string {
  return value instanceof Date ? value.toString() : String(value);
}

export function formatAutomationTime(
  value: AutomationTimeValue,
  { now = new Date(), locale, emptyLabel = 'Unknown' }: AutomationTimeOptions = {},
): AutomationTimeLabel {
