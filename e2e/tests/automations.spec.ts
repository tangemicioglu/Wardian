...
const parameterHeavyBlueprint = {
  schema: 2,
  id: "param-heavy",
  name: "Parameter Heavy Automation",
  nodes: [
    {
      id: "trigger",
      type: "manual_trigger",
...
async function installAutomationsIpcMock(
  page: Page,
  ...
  await page.addInitScript(({ blueprintFixture, eventFixture, workbenchDocument }) => {
    ...
    const tauriWindow = window as Window & {
      __automationsInvokes?: Array<{ command: string; args?: Record<string, unknown> }>;
      __TAURI_INTERNALS__?: Record<string, unknown>;
      __TAURI_EVENT_PLUGIN_INTERNALS__?: Record<string, unknown>;
    };

    tauriWindow.__automationsInvokes = [];
    ...
    tauriWindow.__TAURI_INTERNALS__ = {
      ...
      invoke: async (command: string, args?: Record<string, unknown>) => {
        tauriWindow.__automationsInvokes?.push({ command, args });
...
test("unified Automations view edits, launches, observes, and returns to edit", async ({ page }) => {
  await installAutomationsIpcMock(page);
  await page.setViewportSize({ width: 1700, height: 980 });
  await page.goto("/", { waitUntil: "domcontentloaded" });
  await page.locator('[data-testid="app-shell"]').waitFor({ timeout: 15_000 });

  const titlebar = page.locator(".titlebar");
  await expect(titlebar.getByRole("button", { name: "Automations" })).toHaveCount(0);
  ...
  await expect(page.getByTestId("automations-observe-mode").getByText("run-1")).toBeVisible();
  ...
  await page.getByTestId("automations-view").getByRole("button", { name: "Edit" }).click();
  await expect(page.getByTestId("automations-edit-mode")).toBeVisible();
  await expect(builderNodeById(page, "plan")).toHaveCount(1);
  await expect(builderNodeById(page, "ship")).toHaveCount(1);

  const invokes = await page.evaluate(() => window.__automationsInvokes);
  expect(invokes?.some((call) => call.command === "automation_run")).toBe(true);
  expect(invokes?.some((call) => call.command === "automation_read_run")).toBe(true);
});

test("automation Observe exposes approval controls and records the choice", async ({ page }) => {
  await installAutomationsIpcMock(page);
  await page.setViewportSize({ width: 1700, height: 980 });
  await page.goto("/", { waitUntil: "domcontentloaded" });
  await page.locator('[data-testid="app-shell"]').waitFor({ timeout: 15_000 });
  ...
  const automationView = page.getByTestId("automations-view");
  await expect(automationView.getByRole("button", { name: /^Approve$/ })).toBeVisible();
  await expect(automationView.getByRole("button", { name: /^Reject$/ })).toBeVisible();
  await expect(automationView.getByRole("button", { name: /^Approve$/ })).toHaveCSS("cursor", "pointer");
  fs.mkdirSync("e2e/screenshots/automation-approval", { recursive: true });
  await automationView.screenshot({ path: "e2e/screenshots/automation-approval/observe-approval-actions.png" });

  await automationView.getByRole("button", { name: /^Approve$/ }).click();
  await expect.poll(async () => page.evaluate(() => (
    window.__automationsInvokes?.some((call) => (
      call.command === "automation_approve" && call.args?.granted === true
    ))
  ))).toBe(true);
});

test("automation builder renders persisted loop automation nodes on a visible canvas", async ({ page }) => {
  await installAutomationsIpcMock(page, loopBlueprint);
  await page.setViewportSize({ width: 1700, height: 980 });
  await page.goto("/", { waitUntil: "domcontentloaded" });
  await page.locator('[data-testid="app-shell"]').waitFor({ timeout: 15_000 });
...
test("automation run dialog scrolls parameter-heavy forms within the viewport", async ({ page }) => {
  await installAutomationsIpcMock(page, parameterHeavyBlueprint);
  ...
  fs.mkdirSync("e2e/screenshots/automation-run-dialog", { recursive: true });
  await dialog.screenshot({ path: "e2e/screenshots/automation-run-dialog/scrollable-parameter-form.png" });
});

test("automation monitor fits an adaptive activity card inside a narrow restored workbench surface", async ({ page }) => {
  const referenceSurface = makeWorkbenchSurface("reference-home", "new-tab");
  const automationsSurface = makeWorkbenchSurface("narrow-automations", "automations");
  const restoredDocument = makeWorkbenchDocument({
    revision: 7,
    root: {
      kind: "split",
      node_id: "automation-monitor-narrow-split",
    ...
    groups: {
      ...
      "group-monitor": {
        ...
        surface_ids: [automationsSurface.surface_id],
        active_surface_id: automationsSurface.surface_id,
      },
    },
    surfaces: [referenceSurface, automationsSurface],
  ...
  await installAutomationsIpcMock(page, blueprint, restoredDocument);
  await page.setViewportSize({ width: 1440, height: 800 });
  await page.goto("/", { waitUntil: "domcontentloaded" });
  await page.locator('[data-testid="app-shell"]').waitFor({ timeout: 15_000 });

  const automationsPane = surfacePanel(page, "automations");
  await expect(automationsPane).toBeVisible();
      ...
      schedules: [
        {
          ...
          blueprint_id: "wf",
          name: "Primary restored-pane automation",
          input: {},
          bindings: {},
          assignments: {
        ...
        },
        {
          ...
          name: "Secondary restored-pane automation",
          input: {},
          bindings: {},
  ...
  await automationsPane.getByRole("button", { name: /^monitor$/i }).click();

  const monitor = automationsPane.getByTestId("automation-monitor");
  const activityRegion = monitor.getByRole("region", { name: "Automation activity" });
  const scheduledCards = activityRegion.locator('[data-mode="scheduled"]');
  await expect(scheduledCards).toHaveCount(2);
  const restoredGeometry = await automationsPane.evaluate((pane) => ({
  ...
  const historyCard = activityRegion.getByTestId("automation-history-run-run-monitor-overflow");
  ...
  const geometry = await activityRegion.evaluate((element) => {
    const card = element.querySelector('[data-testid="automation-history-run-run-monitor-overflow"]');
    if (!card) return null;
